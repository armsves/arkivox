...
/** One key: record → index → fetch → owner reveal via Nox. */
export async function runSmoke(ownerKey: Hex) {
  ...
  assert(
    (await decryptTransactionSecret(ownerHandle, fetched!)).token === E2E.token,
    "Private payload token mismatch after fetch",
  );

  log("4/4", "owner reveal amount (Nox gateway)");
  const dec = await decryptTransactionSecret(ownerHandle, transaction);
